...
const Hero = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (prompt.trim()) {
      // Call your API to create the mind map
      setIsLoading(true)
      const res = await fetch('/api/gemini-process', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, userId }) // get userId from auth
      })
      const { data } = await res.json()
      setIsLoading(false)
      // Redirect to the new mind map page
    ...
    <section className='relative flex items-center justify-center min-h-[80vh] overflow-hidden'>
      ...
      <div className='relative z-10 w-full max-w-5xl mx-auto px-4 md:px-16 py-14 bg-white/80 dark:bg-gray-900/80 backdrop-blur-2xl rounded-3xl shadow-2xl border border-blue-100 dark:border-blue-900/40 flex flex-col items-center'>
        ...
        <p className='text-lg md:text-2xl text-blue-700 dark:text-blue-200 mb-10 max-w-4xl mx-auto font-medium text-center'>
          NeuroFlow uses AI to create beautiful, structured mind maps from your
          thoughts. Just type your idea, and watch it branch out into an
        ...
        <div className='flex flex-wrap justify-center gap-3 mt-2'>
          {[
            ...
            >
              Try:{' '}
              <span className='text-blue-600 dark:text-blue-300'>{`"${example}"`}</span>
            </span>
          ))}
        </div>
      </div>
